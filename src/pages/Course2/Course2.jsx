...
<div className='block lg:hidden lg:w-12 w-0.5 mt-4 lg:mt-0 xl:h-0.5 h-12 lg:mx-auto bg-primary_gold' />
   ...
   <div className="flex flex-col justify-center pt-12 items-center flex-wrap -mx-4">
    ...
    
    {
      const rowNumber = Math.floor(index / 3);
      const isLastRow = Math.ceil(chapters.length / 3) === rowNumber + 1;
      const alignment = (Math.floor(index / 3) % 2 === 0) ? 'justify-end ' : 'justify-start ';       
  ...
  const marginClass = rowNumber % 2 === 0 ? 'mr-auto' : 'ml-auto';
    return(
      <React.Fragment key={index}>
      <div key={index}  className={`w-full   justify-center items-center  lg:w-1/2  px-4  mb-4 `}
>
        ...
        id={index}
          ...
          isLocked={item.isLocked}
        />
         { index !== chapters.length - 1 && <div className=' text-center  w-1  -mb-8 mt-6 h-12 text-primary_gold text-lg  bg-primary_gold' >
